...
class DirectMap(nn.Module, map_base):
    def __init__(self, hidden_dim, gaussian_per_patch, out_patch_size, out_image_size, image_size, window_size=1, gs_fix=False):
        ...
        # Cholesky relaed - Keep as a normal attribute to match the original checkpoint
        # Move these tensors to the active device when they are used.
        ...
        self.gau_dict = torch.cat((self.gau_dict, torch.zeros(1,3)), dim=0) # shape:[344,3]
    ...
    def get_iter(self, i):
        ...
        # Use devices with the same size as input loads
        ...
        # Use devices with the same size as input loads
        device = color_.device
        opacity = torch.ones(color_.shape[0], color_.shape[1], 1, device=device)
