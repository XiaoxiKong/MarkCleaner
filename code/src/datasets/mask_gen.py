...
# def random_regular_maskv2(img, ratio=None, min_ratio=0.2, max_ratio=0.6):
#     """Generates a random regular hole"""
#     _, H, W = img.size()
...
#         mask_h = int(H * ratio)
#     else:
#         mask_w = int(W * min_ratio)
#         mask_h = int(H * max_ratio)

#     N_mask = 4
#     total_area = mask_w * mask_h
#     area_per_block = total_area / N_mask

#     n_row = int(math.sqrt(N_mask))
#     n_col = math.ceil(N_mask / n_row)

#     block_w = max(1, mask_w // n_col)
#     block_h = max(1, mask_h // n_row)

#     # ---------- random placement ----------
#     for _ in range(N_mask):
#         x = random.randint(0, H - block_h)
#         y = random.randint(0, W - block_w)
#         mask[:, x:x+block_h, y:y+block_w] = 0
...
# def center_maskv2(img, ratio=None, min_ratio=0.2, max_ratio=0.6):
#     """Generates a center hole with 1/4*W and 1/4*H"""
...
#         ratio_w = min_ratio
#         ratio_h = max_ratio
#         mask_width = int(size[1]*math.sqrt(ratio_w))
#         mask_height = int(size[2]*math.sqrt(ratio_h))
#         x = int((size[1] - mask_width) / 2)
#         y = int((size[2] - mask_height) / 2)
#         range_x = x + mask_width
#         range_y = y + mask_height
...
def random_irregular_mask(img, ratio=None, min_ratio=0.2, max_ratio=0.6):
    ...
    # Convert directly to a tensor to avoid torchvision ToTensor version differences.
    img_mask = torch.tensor(img.astype(np.float32), dtype=torch.float32) / 255.0
    if len(img_mask.shape) == 2:
        img_mask = img_mask.unsqueeze(0)  # Add Channel dimension

    # Repair of bug: The correct logic should be that the place where the line is drawn (img_mask > 0) is a masked area (0) and the place where the line is not drawn is a reserved area (1)
    # The former `img_mask < 1` check made nearly every value true.
    # Pixels without a stroke are kept (1); drawn strokes form the masked region (0).
    mask[0, :, :] = (img_mask[0, :, :] == 0).float()
...
def random_freefrom_mask(img, ratio=None, mv=3, ma=4.0, ml=40, mbw=10, min_ratio=0.2, max_ratio=0.6):
    ...
    img = img.reshape(size[2], size[1])
    # Alternative direct tensor conversion (kept disabled for compatibility):
    # img_mask = torch.tensor(img.astype(np.float32), dtype=torch.float32) / 255.0
    # if len(img_mask.shape) == 2:
